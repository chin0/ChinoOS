...
void kInitializePageTables(void)
{
    ...
    //페이지 디렉터리 포인터 테이블 생성
    //하나의 PDPT로 512GB까지 매핑 가능하므로 하나로 총분함
    //64개의 엔트리를 설정하여 64GB까지 매칭함.
    
    pstPDPTEntry = (PDENTRY*) 0x101000;
    for(i = 0; i < 64; i++)
    {
    ...
    {
        //32비트로는 상위 어드레스를 포현할 수 없으므로, MB단위로 계산한 다음
        //최종 결과를 다시 4GB로 나누어 32비트 이상의 어드레스로 계산함.
